...
public class MultiLayerPerceptron implements Cloneable, Serializable {
	...
	public MultiLayerPerceptron(int[] layers, double learningRate, TransferFunction fun, TransferFunction activ){
		fLearningRate = learningRate;
		fTransferFunction = fun;
		fActivationFunction = activ;
	...
	public double[] forwardPropagation(double[] input){
		...
		for(k = 1; k < fLayers.length; k++){
			for(i = 0; i < fLayers[k].Length; i++){
				...
				if (k == fLayers.length - 1) {
					fLayers[k].Neurons[i].value = new_value;
				} else {
					fLayers[k].Neurons[i].value = fTransferFunction.evalute(new_value, null);					
				}
			}
		}
		...
		for(i = 0; i < fLayers[fLayers.length - 1].Length; i++)
			if (fActivationFunction != null) {
				output[i] = fActivationFunction.evalute(fLayers[fLayers.length - 1].Neurons[i].value, values);				
			} else {
				output[i] = fTransferFunction.evalute(fLayers[fLayers.length - 1].Neurons[i].value, values);
			}
		
		return output;
	}
	
	public double[] forwardPropagation_2(double[] input){
		int i, j, k;
		double new_value;
		double output[] = new double[fLayers[fLayers.length - 1].Length];
	...
	public double backPropagate(double[] input, double[] output){
		...
		for(i = 0; i < fLayers[fLayers.length - 1].Length; i++)
			fLayers[fLayers.length - 1].Neurons[i].delta = (output[i] - new_output[i]) * fTransferFunction.evaluteDerivate(new_output[i], null);
		...
		for(k = fLayers.length - 2; k >= 0; k--){
			for(i = 0; i < fLayers[k].Length; i++){
				...
				fLayers[k].Neurons[i].delta = error * fTransferFunction.evaluteDerivate(fLayers[k].Neurons[i].value, null);				
			}
	...
	TransferFunction  fActivationFunction;
}
